"""
完整的生物信息学富集分析报告生成脚本
执行超幾何分佈檢定 (Hypergeometric Test) - 只计算 Enrichment
生成包含多重检验校正、比值和对数转换的完整报告
"""
...
output_file = r"C:\Users\yukan\Downloads\COSBI_Lab\enrich_project\HW42\d14_fgg_enrichment_full.csv"

print("=" * 80)
print("完整富集分析報告生成器")
print("超幾何分佈檢定 (Hypergeometric Test) - Enrichment Only")
print("=" * 80)
...
for idx, row in feature_df.iterrows():
    ...
    pvalues_list.append(enrichment_pvalue)
    ...
    results.append({
        ...
        'Enrichment Fold Change': fold_change,
...
for i, result in enumerate(results):
    result['FDR enriched P-value'] = fdr_pvalues[i]
    result['Bonferroni enriched P-value'] = bonf_pvalues[i]
...
for result in results:
    # log2(Fold Change)
    fc = result['Enrichment Fold Change']
    if fc > 0:
    ...
    # -log10(FDR P-value)
    fdr_pval = result['FDR enriched P-value']
    if fdr_pval > 0:
...
print(f"  • FDR < 0.05:          {(results_df['FDR enriched P-value'] < 0.05).sum()} 個")
print(f"  • FDR < 0.01:          {(results_df['FDR enriched P-value'] < 0.01).sum()} 個")
print(f"\nBonferroni 校正:")
print(f"  • Bonf < 0.05:         {(results_df['Bonferroni enriched P-value'] < 0.05).sum()} 個")
print(f"  • Bonf < 0.01:         {(results_df['Bonferroni enriched P-value'] < 0.01).sum()} 個")
print(f"\nFold Change 統計:")
print(f"  • 最小:                {results_df['Enrichment Fold Change'].min():.4f}")
print(f"  • 最大:                {results_df['Enrichment Fold Change'].max():.4f}")
print(f"  • 平均:                {results_df['Enrichment Fold Change'].mean():.4f}")
print("=" * 80)

# 顯示前幾個最顯著的特徵
print("\n前 10 個最顯著的特徵 (按 P-value):")
print(results_df.head(10)[['Feature Name', 'Observed Ratio', 'Enrichment Fold Change', 'P-value', 'FDR enriched P-value']].to_string(index=False))
